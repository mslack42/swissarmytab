    ...
    toggleEnabled: (state, action: PayloadAction<BodyEditorId>) => {
      ...
      if (newList.includes(action.payload)) {
        ...
        if (state.favouriteEditors.includes(action.payload)) {
          state.favouriteEditors = state.favouriteEditors.filter(
            (e) => e != action.payload,
          );
        }
      } else {
...
export const isFavouriteEditor: (
  bodyEditorId: BodyEditorId,
) => (state: RootState) => boolean =
  (bodyEditorId: BodyEditorId) => (state: RootState) =>
    state.editorConfigs.favouriteEditors.includes(bodyEditorId);
export const isEnabledEditor: (
  bodyEditorId: BodyEditorId,
) => (state: RootState) => boolean =
  (bodyEditorId: BodyEditorId) => (state: RootState) =>
    state.editorConfigs.enabledEditors.includes(bodyEditorId);
export const favouriteEditors = (state: RootState) =>
  state.editorConfigs.favouriteEditors;
export const enabledEditors = (state: RootState) =>
  state.editorConfigs.enabledEditors;
