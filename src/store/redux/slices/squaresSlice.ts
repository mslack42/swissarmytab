...
export const squaresSlice = createSlice({
  ...
  reducers: {
    ...
    deletePanels: (state, action: PayloadAction<string[]>) => {
      state.panelData = state.panelData.filter(
...
export const selectPanelData: (
  panelId: string,
) => (state: RootState) => SquareData | undefined =
  (panelId: string) => (state: RootState) =>
    state.squares.panelData.find((pd: SquareData) => pd.id == panelId);
export const selectSquares: (state: RootState) => SquaresState = (
  state: RootState,
) => state.squares;
export const selectHasPluralPanels = (state: RootState) =>
  state.squares.panelData.length > 1;
