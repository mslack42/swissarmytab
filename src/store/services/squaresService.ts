...
export class SquaresService {
  ...
  renameSquare = (id: string, newName: string, api: DockviewPanelApi) => {
    const nameExists = store
      .getState()
      .squares.panelData.some((p) => p.title == newName);
  ...
  dupeSquare = (panelApi: DockviewPanelApi, containerApi: DockviewApi) => {
    ...
    const oldPanelData = store
      .getState()
      .squares.panelData.filter((p) => p.id == panelApi.id)[0];
    store.dispatch(
      addPanel({
        id: newPanelId,
        name: newPanelName,
        componentTypeId: oldPanelData.componentTypeId,
        dataId: newDataId,
      }),
    );
